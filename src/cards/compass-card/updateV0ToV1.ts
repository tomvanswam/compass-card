...
export function configV0ToV1(dep: CompassCardConfigV0): CompassCardConfigV1 {
  const conf: CompassCardConfigV1 = {
    type: "custom:compass-card",
    indicator_sensors: [{ sensor: dep.entity }],
  };

  if (
    dep.compass?.indicator &&
    INDICATORS.indexOf(dep.compass?.indicator) !== DEFAULT_INDICATOR
  ) {
    conf.indicator_sensors[0] = {
      ...conf.indicator_sensors[0],
      indicator: { type: dep.compass.indicator },
    };
  }
  ...
  if (dep.direction_offset && dep.direction_offset !== "0") {
    conf.compass = {
      ...conf.compass,
      north: { offset: Number(dep.direction_offset), ...conf.compass?.north },
    };
  }
  if (dep.compass?.show_north && dep.compass?.show_north) {
    conf.compass = {
      ...conf.compass,
      north: { show: true, ...conf.compass?.north },
    };
  }
