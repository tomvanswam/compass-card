...
var common$1 = {
	version: "Versie",
	description: "Toon een kompas met een pijl wijzend naar de waarde van de entity",
	invalid_configuration: "Foutieve configuratie",
	no_entity: "Entity niet geconfigureerd",
	offset_not_a_number: "Direction offset is geen nummer",
	invalid: "ongeldig",
	on: "Aan",
	off: "Uit"
};
...
var common$2 = {
	version: "Versione",
	description: "Mostra una bussola con un indicatore nella direzione indicata dal valore dell'entità.",
	invalid_configuration: "Configurazione non valida",
	no_entity: "Entità non configurata",
	offset_not_a_number: "L'offset della direzione non è un numero.",
	invalid: "invalido",
	on: "Acceso",
	off: "Spento"
};
...
const COMPASS_LANGUAGES = Object.keys(languages);
function getLocalLanguage() {
    return (localStorage.getItem('selectedLanguage') || 'en').replace(/['"]+/g, '').replace('-', '_');
}
function localize(string, search = '', replace = '', language = '') {
    let translated;
    if (language === '') {
...
const UNAVAILABLE = localize('common.invalid');
const INDICATORS = ['arrow_outward', 'arrow_inward', 'circle'];
const CONFIG_ENTITY = 'entity';
const CONFIG_SECONDARY_ENTITY = 'secondary_entity';
const CONFIG_COMPASS = 'compass';
const CONFIG_INDICATOR = 'indicator';
const CONFIG_DIRECTION_OFFSET = 'direction_offset';
const CONFIG_NAME = 'name';
const CONFIG_SHOW_NORTH = 'show_north';
const CONFIG_LANGUAGE = 'language';
const CONFIG_DOMAINS = ['sensor', 'input_number', 'input_text'];

let CompassCardEditor = class CompassCardEditor extends LitElement {
    setConfig(config) {
        this._config = config;
    }
    get _name() {
    ...
    get _secondary_entity() {
        if (this._config) {
            return this._config.secondary_entity || '';
        }
        return '';
    }
    get _direction_offset() {
        if (this._config) {
            return this._config.direction_offset || '0';
        }
        return '0';
    }
    get _compass_indicator() {
        var _a, _b;
        if (this._config) {
            return ((_b = (_a = this._config) === null || _a === void 0 ? void 0 : _a.compass) === null || _b === void 0 ? void 0 : _b.indicator) || INDICATORS[1];
        }
        return INDICATORS[1];
    }
    get _compass_show_north() {
        var _a, _b;
        if (this._config) {
            return ((_b = (_a = this._config) === null || _a === void 0 ? void 0 : _a.compass) === null || _b === void 0 ? void 0 : _b.show_north) || false;
        }
        return false;
    }
    get _compass_language() {
        var _a, _b;
        if (this._config) {
            return ((_b = (_a = this._config) === null || _a === void 0 ? void 0 : _a.compass) === null || _b === void 0 ? void 0 : _b.language) || getLocalLanguage();
        }
    ...
    getValue(item, ancestor) {
        ...
        if (dotLoc > -1) {
            ...
            return Object.assign(Object.assign({}, ancestor), { [parent]: Object.assign(Object.assign({}, ancestor[parent]), { [child]: item.checked !== undefined ? item.checked : item.value }) });
        }
        else {
            return Object.assign(Object.assign({}, ancestor), { [item.configValue]: item.checked !== undefined ? item.checked : item.value });
        }
    }
    _valueChanged(ev) {
...
], CompassCardEditor.prototype, "hass", void 0);
__decorate([
    internalProperty()
], CompassCardEditor.prototype, "_config", void 0);
CompassCardEditor = __decorate([
...
let CompassCard = CompassCard_1 = class CompassCard extends LitElement {
    ...
    render() {
        ...
        if (!Number.isNaN(Number(this._config.direction_offset))) {
            direction_offset = CompassCard_1.positiveDegrees(+this._config.direction_offset);
        }
        const direction = this.hass.states[this._config.entity];
        const secondary_entity = this._config.secondary_entity ? this.hass.states[this._config.secondary_entity] : undefined;
    ...
    renderCompass(direction, secondary, direction_offset) {
        var _a, _b, _c;
        // default to North
        let degrees = 0;
        ...
        if (Number.isNaN(Number(directionStr))) {
            degrees = CompassCard_1.getDegrees(directionStr);
            abbreviation = directionStr;
            if (degrees === -1) {
                const matches = directionStr.replace(/\s+/g, '').match(/[+-]?\d+(\.\d)?/);
                if (matches === null || matches === void 0 ? void 0 : matches.length) {
                    degrees = CompassCard_1.positiveDegrees(parseFloat(matches[0]));
                }
                else {
                    degrees = 0;
                }
                abbreviation = CompassCard_1.getCompassAbbreviation(degrees, (_a = this._config.compass) === null || _a === void 0 ? void 0 : _a.language);
            }
        }
        else {
            degrees = CompassCard_1.positiveDegrees(parseFloat(directionStr));
            abbreviation = CompassCard_1.getCompassAbbreviation(degrees, (_b = this._config.compass) === null || _b === void 0 ? void 0 : _b.language);
        }
        return html `
      <div class="compass">
        ...
        <div class="indicator ${CompassCard_1.computeIndicator(this._config)}" style="transform: rotate(${(degrees + direction_offset) % 360}deg)"></div>
        ${((_c = this._config.compass) === null || _c === void 0 ? void 0 : _c.show_north) ? html `<div class="indicator north" style="transform: rotate(${CompassCard_1.positiveDegrees(direction_offset)}deg)"></div>` : ''}
      </div>
    `;
    }
    getConfigStyle(style) {
    ...
    static positiveDegrees(degrees) {
        return degrees < 0 ? degrees + (Math.abs(Math.ceil(degrees / 360)) + 1) * 360 : degrees % 360;
    }
};
__decorate([
    property({ attribute: false })
], CompassCard.prototype, "hass", void 0);
__decorate([
    property({ attribute: false })
], CompassCard.prototype, "_config", void 0);
CompassCard = CompassCard_1 = __decorate([
